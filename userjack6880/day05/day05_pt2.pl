...
foreach my $line (<$fh>) {
  ...
  if ($phase == 0) {
    ...
    my @top_items;                               # top x items of from stack
    for (my $i = $top_from; $i > $top_from - $move[1]; $i--) {
      push(@top_items,$stacks{$move[3]}{$i});
    }
    $top_to = $top_to + $move[1];                            # move index of to stack up
    $stacks{$move[5]}{'top'} = $top_to;          # change the stored top index
    for (my $i = $top_to-$move[1]+1; $i < $top_to+1; $i++) {
      $stacks{$move[5]}{$i} = pop(@top_items);
    }                                            # save value of top items in to stack
    $top_from = $top_from - $move[1];                          # move index of from stack down
